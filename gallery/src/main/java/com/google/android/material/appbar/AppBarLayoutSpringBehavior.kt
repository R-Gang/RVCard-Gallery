...
class AppBarLayoutSpringBehavior : AppBarLayout.Behavior {
    ...
    override fun onStartNestedScroll(
        ...
        val started = super.onStartNestedScroll(parent,
            child,
            directTargetChild,
            target,
            nestedScrollAxes,
    ...
    override fun onNestedScroll(
        ...
        if (dyUnconsumed < 0) {
            setHeaderTopBottomOffset(coordinatorLayout,
                child,
                topBottomOffsetForScrollingSibling - dyUnconsumed,
    ...
    private fun animateFlingSpring(
        coordinatorLayout: CoordinatorLayout,
        abl: AppBarLayout,
        originNew: Int,
    ) {
        if (mFlingAnimator == null) {
            ...
            mFlingAnimator!!.addUpdateListener { animation ->
                updateSpringHeaderHeight(coordinatorLayout,
                    abl,
    ...
    private fun animateRecoverBySpring(coordinatorLayout: CoordinatorLayout, abl: AppBarLayout) {
        if (mSpringRecoverAnimator == null) {
            ...
            mSpringRecoverAnimator!!.addUpdateListener { animation ->
                updateSpringHeaderHeight(coordinatorLayout,
                    abl,
                    animation.animatedValue as Int)
            }
    ...
    override fun onMeasureChild(
        ...
        val b = super.onMeasureChild(parent,
            child,
    ...
    fun getHeaderExpandedHeight(appBarLayout: AppBarLayout): Int {
        var range = 0
        var i = 0
    ...
    private fun snapToChildIfNeeded(coordinatorLayout: CoordinatorLayout, abl: AppBarLayout) {
        ...
        if (offsetChildIndex >= 0) {
            ...
            if (flags and AppBarLayout.LayoutParams.FLAG_SNAP == AppBarLayout.LayoutParams.FLAG_SNAP) {
                ...
                val newOffset = if (offset < (snapBottom + snapTop) / 2) snapBottom else snapTop
    ...
    public override fun setHeaderTopBottomOffset(
        coordinatorLayout: CoordinatorLayout,
        appBarLayout: AppBarLayout, newOffset: Int, minOffset: Int, maxOffset: Int,
    ): Int {
        return setHeaderTopBottomOffset(coordinatorLayout,
            appBarLayout,
            newOffset,
            minOffset,
            maxOffset,
        ...
        if (minOffset != 0 && curOffset >= minOffset && curOffset <= maxOffset) {
            ...
            if (curOffset != newOffset) {
                val interpolatedOffset =
                    if (appBarLayout.hasChildWithInterpolator()) interpolateOffset(appBarLayout,
                        newOffset) else newOffset
                ...
                appBarLayout.onOffsetChanged(topAndBottomOffset)
    ...
    private fun updateSpringByScroll(
        coordinatorLayout: CoordinatorLayout,
        ...
        if (appBarLayout.height >= mPreHeadHeight && type == 1) {
            if (mFlingAnimator == null) animateFlingSpring(coordinatorLayout,
                appBarLayout,
                originNew)
            return originNew
        }
    ...
    public override fun getTopBottomOffsetForScrollingSibling(): Int {
    ...
    private fun updateSpringOffsetByscroll(
        coordinatorLayout: CoordinatorLayout,
        appBarLayout: AppBarLayout,
        offset: Int,
    ) {
    ...
    private fun updateSpringHeaderHeight(
        coordinatorLayout: CoordinatorLayout,
        appBarLayout: AppBarLayout,
        offset: Int,
    ) {
    ...
    @VisibleForTesting
    public override fun isOffsetAnimatorRunning(): Boolean {
    ...
    private fun updateAppBarLayoutDrawableState(
        ...
        if (child != null) {
            ...
            if (flags and AppBarLayout.LayoutParams.SCROLL_FLAG_SCROLL != 0) {
                ...
                if (direction > 0 && flags and (AppBarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS
                            or AppBarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS_COLLAPSED) != 0
                ) {
    ...
    companion object {
        private const val TAG = "SpringBehav"
